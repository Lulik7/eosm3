...
import {
    ...
    Alert, Snackbar, List, ListItem, ListItemIcon, ListItemText,
...
import axios from 'axios'; // Убедись, что axios установлен: npm install axios

// --- Types (Обновлено под твою базу) ---
...
const EngineerPage: React.FC<EngineerPageProps> = ({ logout }) => {
    ...
    const fetchTasks = async () => {
        try {
            ...
            console.log("Что пришло из базы:", response.data); // Это поможет увидеть структуру в консоли

            // Если бэкенд прислал объект, ищем массив внутри него
            const result = response.data;

            if (Array.isArray(result)) {
                setTasks(result);
            } else if (result && typeof result === 'object') {
                // Если данные лежат внутри поля 'incidents' или 'data'
                const possibleArray = result.incidents || result.data || Object.values(result).find(val => Array.isArray(val));
                if (Array.isArray(possibleArray)) {
                    setTasks(possibleArray);
                } else {
                    setTasks([]);
    ...
    const deleteTask = async (id: string) => {
        if (window.confirm("Are you sure you want to delete this incident from DB?")) {
            try {
                await axios.delete(`http://localhost:5000/api/incidents/${id}`, { withCredentials: true });
    ...
    const updateStatus = async (taskId: string, newStatus: string) => {
        try {
            await axios.patch(`http://localhost:3000/api/incidents/${taskId}`, { status: newStatus }, { withCredentials: true });
            setTasks(tasks.map(t => t._id === taskId ? { ...t, status: newStatus as any } : t));
            setStatusAnchor(null);
            showMessage(`Status updated in DB to ${newStatus}`);
        } catch (error) {
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ flexGrow: 1, p: 3 }}>
                {loading ? (
                    <Box sx={{ display: 'flex', justifyContent: 'center', mt: 10 }}><CircularProgress sx={{ color: '#e67e22' }} /></Box>
                ) : (
            ...
            <Dialog open={openInfoDialog} onClose={() => setOpenInfoDialog(false)} fullWidth maxWidth="sm">
                <DialogTitle sx={{ bgcolor: '#1a1a1a', color: '#e67e22', fontWeight: 900 }}>DETAILS: {selectedTask?.incidentNumber}</DialogTitle>
                <DialogContent sx={{ mt: 2 }}>
                    ...
                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>Severity: {selectedTask?.severity.toUpperCase()}</Typography>
                </DialogContent>
                <DialogActions><Button onClick={() => setOpenInfoDialog(false)}>Close</Button></DialogActions>
            </Dialog>

            {/* Остальные диалоги (без изменений дизайна) */}
            <Dialog open={openUpdate} onClose={() => setOpenUpdate(false)} fullWidth maxWidth="xs">
                ...
                <DialogContent sx={{ mt: 3 }}><TextField fullWidth multiline rows={3} label="Maintenance Notes" variant="outlined" /></DialogContent>
                <DialogActions><Button onClick={() => setOpenUpdate(false)}>Cancel</Button><Button variant="contained" onClick={() => {setOpenUpdate(false); showMessage("Report Sent");}} sx={{ bgcolor: '#e67e22' }}>Submit</Button></DialogActions>
            </Dialog>
            ...
            <Dialog open={openSupportDialog} onClose={() => setOpenSupportDialog(false)} fullWidth maxWidth="xs">
                ...
                <DialogContent sx={{ mt: 3 }}><TextField fullWidth multiline rows={3} label="Message" variant="outlined" /></DialogContent>
                <DialogActions><Button onClick={() => setOpenSupportDialog(false)}>Cancel</Button><Button variant="contained" onClick={() => {setOpenSupportDialog(false); showMessage("Sent to Support");}} sx={{ bgcolor: '#2b4d7e' }}>Send</Button></DialogActions>
            </Dialog>
            ...
            <Dialog open={openAdminDialog} onClose={() => setOpenAdminDialog(false)} fullWidth maxWidth="xs">
                ...
                <DialogContent sx={{ mt: 3 }}><TextField fullWidth multiline rows={3} label="Details" variant="outlined" /></DialogContent>
                <DialogActions><Button onClick={() => setOpenAdminDialog(false)}>Cancel</Button><Button variant="contained" onClick={() => {setOpenAdminDialog(false); showMessage("Emergency report sent");}} sx={{ bgcolor: '#d32f2f' }}>Send</Button></DialogActions>
            </Dialog>
