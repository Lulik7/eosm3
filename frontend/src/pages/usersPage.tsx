...
const UserPage: React.FC<UserPageProps> = ({ logout }) => {
    ...
    const ticketScrollStyle = {
        flexGrow: 1, maxHeight: '400px', overflowY: 'auto', pr: 1,
        '&::-webkit-scrollbar': { width: '4px' },
        '&::-webkit-scrollbar-track': { background: 'transparent' },
    ...
    return (
        <Box sx={{
            ...
            <Grid container sx={{ flexGrow: 1, m: 0, display: 'flex', flexWrap: 'nowrap' }}>
                        ...
                        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 6 }}>
                            <Stack direction="row" spacing={2} alignItems="center">
                                <Avatar onClick={() => setOpenAvatarDialog(true)} sx={{ bgcolor: '#2b4d7e', width: 64, height: 64, border: '3px solid white', cursor: 'pointer', boxShadow: 3 }}>
                                    {React.isValidElement(selectedAvatar) ? React.cloneElement(selectedAvatar as React.ReactElement<any>, { sx: { fontSize: 40 } }) : selectedAvatar}
                                </Avatar>
                                <Box>
                                    <Stack direction="row" alignItems="center" spacing={1}>
                                        <Typography variant="h5" sx={{ color: '#2b4d7e', fontWeight: 900 }}>{profile.name}</Typography>
                                        <IconButton size="small" onClick={() => { setTempProfile({...profile}); setOpenEditDialog(true); }}>
                                            <EditIcon sx={{ fontSize: 18, color: '#2b4d7e' }} />
                                        </IconButton>
                                    </Stack>
                                    <Stack spacing={0.2}>
                        ...
                        <Grid container spacing={3} justifyContent="space-between" sx={{ mb: 2 }}>
                                        ...
                                        <List disablePadding>
                                            {[
                                                ...
                                                <React.Fragment key={i}>
                                                    <ListItem disableGutters sx={listItemStyle} onClick={() => handleOpenMail(mail.t, mail.m)}>
                            ...
                            <Grid item xs={12} md={4}>
                                <Paper elevation={10} sx={{ p: 4, borderRadius: 0, borderTop: '5px solid #2b4d7e' }}>
                                    ...
                                    <Stack spacing={3}>
                                        <TextField fullWidth label="Subject" variant="outlined" value={subject} onChange={(e) => setSubject(e.target.value)} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 0 } }} />
                                        ...
                                        <TextField fullWidth multiline rows={4} label="Problem" value={ticketText} onChange={(e) => setTicketText(e.target.value)} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 0 } }} />
                                        <Button variant="contained" startIcon={<SendIcon />} onClick={handleSendTicket} sx={{ bgcolor: '#2b4d7e', py: 1.5, borderRadius: 0, fontWeight: 'bold' }}>SEND</Button>
                                    </Stack>
                                </Paper>
                            </Grid>
                        </Grid>
                    ...
                    <Box sx={{
                        ...
                        <List disablePadding>
                            {[
                                ...
                                <ListItem key={idx} sx={{
                                    mb: 0.5, px: 1, borderRadius: 0, cursor: 'pointer', transition: '0.2s',
                ...
                <Box sx={{ display: 'flex', whiteSpace: 'nowrap', animation: 'marquee 45s linear infinite', '&:hover': { animationPlayState: 'paused' } }}>
                    {[1, 2].map((group) => (
                        <Stack key={group} direction="row" spacing={10} alignItems="center" sx={{ color: 'white', px: 2 }}>
                            {[
                                "NEW SOLAR PANEL REBATES: SAVE 20% ON INSTALLATION!",
                                "CITY MARATHON 2026: REGISTRATION IS OPEN!",
                                "HIGH-SPEED FIBER NOW IN BAKER STREET AREA.",
                                "FREE PARKING FOR ELECTRIC VEHICLES IN CENTER ZONE.",
                                "NEW COMMUNITY CENTER OPENING IN NORTH DISTRICT - JUNE 15.",
                                "UPGRADE YOUR WASTE MANAGEMENT PLAN VIA SMART APP.",
                                "MUNICIPAL TAX BENEFITS FOR LOCAL STARTUPS.",
                                "RECYCLING WEEK: GET EXTRA POINTS ON YOUR CITY CARD!"
                            ].map((text, i) => (
                                <Typography key={i} variant="body2" sx={{ fontWeight: 800, letterSpacing: 1.2, display: 'flex', alignItems: 'center' }}>
                                    <AdIcon sx={{ fontSize: 18, mr: 1, color: '#f5a623' }} /> {text}
                                </Typography>
                            ))}
                        </Stack>
                    ))}
